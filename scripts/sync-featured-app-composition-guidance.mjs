...
const DOC_ID = "1NCenLXg6w80W5NUmJvyIbV1vUp1QlFgwS-UxINwjlXc";
...
const outputPath = path.join(process.cwd(), "featured-app-composition-guidance.html");

const response = await fetch(EXPORT_URL, {
  headers: {
    "User-Agent": "canton-featured-app-composition-guidance-sync/1.0",
  },
});

if (!response.ok) {
  throw new Error(
    `Failed to fetch Google Doc export: ${response.status} ${response.statusText}. ` +
      "Confirm the source Google Doc is shared as Anyone with the link can view."
  );
}
...
<head>
  ...
  <title>Guidance on Composition and Related Parties | Canton Network</title>

  <style>
    :root {
      --panel-soft: rgba(11, 8, 40, 0.78);
    ...
    .doc-content td {
      padding: 14px 12px;
      border-top: 1px solid rgba(126, 92, 186, 0.22);
      text-align: left;
      vertical-align: top;
    }
