...
const DOC_ID = "1QlkQx4_lGfwsvpZPRuWvamLAsNrXlj01AExf0gTaXTY";
...
const outputPath = path.join(process.cwd(), "sv-milestone-framework.html");

const response = await fetch(EXPORT_URL, {
  headers: {
    "User-Agent": "canton-sv-milestone-framework-sync/1.0",
  },
});
...
<head>
  ...
  <style>
    :root {
      --bg: #060815;
      --panel: rgba(16, 23, 42, 0.94);
      --panel-soft: rgba(22, 32, 58, 0.88);
      --text: #f7f8fc;
      --muted: #c5ccdc;
      --line: rgba(156, 182, 255, 0.2);
      --accent: #78f0c5;
      --accent-strong: #bafc68;
    ...
    body {
      ...
      min-height: 100%;
      background:
        radial-gradient(circle at top left, rgba(120, 240, 197, 0.16), transparent 30%),
        radial-gradient(circle at top right, rgba(186, 252, 104, 0.12), transparent 24%),
        linear-gradient(180deg, #070b17 0%, #060815 100%);
      color: var(--text);
    ...
    .eyebrow {
      ...
      padding: 8px 12px;
      border-radius: 999px;
      background: rgba(120, 240, 197, 0.12);
      color: var(--accent);
      font-size: 13px;
      font-weight: 700;
    ...
    .button-secondary {
      ...
      border: 1px solid rgba(186, 252, 104, 0.3);
      color: var(--text);
    }
